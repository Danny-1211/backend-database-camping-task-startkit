...
-- 2-2. 新增：在 `CREDIT_PURCHASE` 資料表，新增三筆資料：（請使用 name 欄位做子查詢）
    -- 1. `王小明` 購買 `14 堂組合包方案`
    -- 2. `王小明` 購買 `21 堂組合包方案`
    -- 3. `好野人` 購買 `14 堂組合包方案`id	user_id	credit_package_id	purchased_credits	price_paid	created_at	purchase_at
    INSERT INTO "CREDIT_PURCHASE" (user_id, price_paid, credit_package_id, purchased_credits)
    VALUES(
        (SELECT id FROM "USER" WHERE name = '王小明'),
        (SELECT price FROM "CREDIT_PACKAGE" WHERE name = '14'),
        (SELECT id FROM "CREDIT_PACKAGE" WHERE name = '14'),
        (SELECT credit_amount FROM "CREDIT_PACKAGE" WHERE name = '14')
    ),
    (
        (SELECT id FROM "USER" WHERE name = '王小明'),
        (SELECT price FROM "CREDIT_PACKAGE" WHERE name = '21'),
        (SELECT id FROM "CREDIT_PACKAGE" WHERE name = '21'),
        (SELECT credit_amount FROM "CREDIT_PACKAGE" WHERE name = '21')
    ),
    (
...
-- 5-5. 修改：`王小明` 現在已經加入直播室了，請在`COURSE_BOOKING`更新該筆預約資料（請注意，不要更新到已經取消的紀錄）：
    ...
    -- 2. 狀態`status` 設定為上課中
    UPDATE  "COURSE_BOOKING" SET join_at = '2024-11-25 14:01:59', status = '上課中' WHERE status NOT IN ('課程已取消') AND user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io');
...
-- 5-7. 查詢：計算用戶王小明的已使用堂數，顯示須包含以下欄位： user_id , total。 (需使用到 Count 函式與 Group By)
    ...
    FROM "COURSE_BOOKING"
    WHERE status NOT IN ('課程已取消') AND user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io')
    Group by  user_id;
-- 5-8. [挑戰題] 查詢：請在一次查詢中，計算用戶王小明的剩餘可用堂數，顯示須包含以下欄位： user_id , remaining_credit
    ...
    SELECT SUM(CP.purchased_credits) - COUNT(CB.*)
    FROM "COURSE_BOOKING"  AS CB INNER JOIN "CREDIT_PURCHASE" AS CP ON CB.user_id = CP.user_id
    WHERE status = '上課中'  AND CP.user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io');
...
-- 顯示須包含以下欄位： 專長名稱, coach_total
    SELECT Count(coach_id) AS coach_total, MAX(S.name) AS 專長名稱
    FROM "SKILL" AS S
